...
def main(args):
    ...
    with mlflow.start_run():
        ...
        # Drop the feature schema *inside* the model folder as well, so the model
        # directory is self-contained: whatever the serving side points MODEL_DIR
        # at, feature_columns.txt sits right next to it. (Must run after
        # log_model, which creates the folder.)
...
if __name__ == "__main__":
    ...
    p.add_argument("--input", type=str, default="data/raw/Telco-Customer-Churn.csv",
                   help="path to CSV (default: data/raw/Telco-Customer-Churn.csv)")
...
#
# --input defaults to data/raw/Telco-Customer-Churn.csv, so a bare
# `python scripts/run_pipeline.py` works too.
