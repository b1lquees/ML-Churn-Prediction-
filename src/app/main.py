...
# Used by the container HEALTHCHECK and any uptime monitoring.
@app.get("/health")
def health():
    """
...
# === LANDING PAGE ===
# Send the site root to the Gradio UI. On Hugging Face Spaces (and any other
# host) visitors open "/" first, and a JSON health blob is a poor front door.
# The health check lives at /health instead.
...
class CustomerData(BaseModel):
    ...
    SeniorCitizen: int = 0     # 0 or 1 - the model was trained on this feature
...
def get_prediction(data: CustomerData):
    ...
    Expected Response:
    - 200 {"prediction": "Likely to churn"} or {"prediction": "Not likely to churn"}
    - 500 {"detail": "error_message"} if prediction fails
    """
    try:
        # model_dump() is the Pydantic v2 spelling; .dict() is deprecated
        result = predict(data.model_dump())
        return {"prediction": result}
    except Exception as e:
        # Surface failures as a real 5xx. Returning 200 with an {"error": ...}
        # body hides outages from clients, monitoring and the ALB health checks.
...
def gradio_interface(
    ...
    data = {
        "gender": gender,
        "SeniorCitizen": 1 if str(SeniorCitizen) == "Yes" else 0,
        "Partner": Partner,
